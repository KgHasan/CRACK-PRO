...
def started():
    ...
    print('%s [%s•%s] %s[%sHASAN✓-OK%s] IDS SAVED IN >> %sHASAN✓-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sHASAN❌-CP%s] IDS SAVED IN >> %sHASAN❌-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [3 SEC] EVERY 3 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("HASAN❌CP")
    except:pass
    try:os.mkdir("HASAN✓-OK")
    except:pass
...
def genkey():
	try:
		...
		print("\033[0;94m          Facebook ID : Hasa N")
		print("\033[0;94m          Facebook Group : Kg Spamming team\n\n")
		subprocess.check_output(["am", "start", "https://wa.me/+8801996636652"])
		exit()
	except(IOError):
		rb = rx
